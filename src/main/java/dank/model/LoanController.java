	...
	public ModelAndView loanstart(int lc_num, String ac_num, HttpSession session) {
		...
		if (bangkingdao.trtrAcChk("9001111111") >= 1) {
			if (bangkingdao.trtrAcChk(ac_num) >= 1) {
				if (Long.parseLong(bangkingdao.trbalChkadmin(my_tr)) >= Long.parseLong(
					String.valueOf(vo.getLr_amount()))) {
		...
		if (vo.getType() == 1) {
			...
			for (int i = 0; i < totalterm; i++) {
				...
				if (i < g) {
					...
					v.setRepayMR(
						(int)(((m * r / 12) * (Math.pow((1 + r / 12), n))) / ((Math.pow((1 + r / 12), n)) - 1)));
			...
			for (LoanFileVO e : vo.getList()) {
				if (e.getMfile().getOriginalFilename() != "") {
					String path = r_path
						...
						+ vo.getLc_num()
						+ "\\"
						+ e.getMfile().getOriginalFilename();
					ff = new File(path);
